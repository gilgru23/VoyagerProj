...
export default function Bluetooth({ navigation }) {
  ...
  const bluetoothScan = async () => {
    ...
    try {
      const devices = await RNBluetoothClassic.getBondedDevices({ delimiter: '\r' });
      await RNBluetoothClassic.accept({ delimiter: '\r' });
  ...
  return (
    <View style={styles.container}>
      ...
      {
        bondedDevices.map(device => <Button title={device.name} onPress={() => connect(device)} style={{marginBottom:"20%"}}/>)
      }
    </View>
